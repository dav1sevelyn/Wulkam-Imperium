...
class LooseScreen extends StatelessWidget {
  final LooseType type;
  const LooseScreen({Key? key, required this.type}) : super(key: key);
                      ...
                      children: [
                        InkWell(
                          onTap: () async {
                            ...
                            if(type==LooseType.d50){
                              await box.clear();
                              user.balance=((user.balance!)/2).round();
                              await box.put('user', user);
                            }
                            if(type==LooseType.all){
                              await box.clear();
                              user.balance=0;
                              await box.put('user', user);
                            }
                            if(type==LooseType.m1000){
                              await box.clear();
                              user.balance=user.balance!-1000;
                              await box.put('user', user);
                            }
                            if(type==LooseType.m500){
                              await box.clear();
  ...
  Widget selector() {
    switch (type) {
      case LooseType.m1000:return Row(
            children: [
              Text(
                '-1000',
      ...
      case LooseType.m500:return Row(
            children: [
              Text(
                '-500',
                style: AppTypography.main
